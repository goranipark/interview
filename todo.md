...
### Phase 5 — 탭 4: 기자증 & 보고서
- [ ] 기자증 카드 (이름 입력 연동, 완료 기관 수 배지)
- [ ] 학습지 내용 취합해 보고서 요약 자동 생성
- [ ] 인쇄 버튼 (`window.print()`) + 인쇄용 CSS(@media print)

### Phase 6 — 마감
- [ ] design.md 기준 전체 스타일 점검 (색상 대비, 폰트 크기)
- [ ] 모바일/태블릿 가로 레이아웃 확인 (학교 태블릿 사용 가능성 고려)
- [ ] `npm run build`로 정적 파일 생성 → 오프라인(서버 없이) 정상 동작 확인
- [ ] 학생용 배포 방법 결정: 학교 PC 로컬 파일 / 학급 USB / GitHub Pages 등 정적 호스팅 중 선택
